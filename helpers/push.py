        ...
        for i, file in enumerate(files):
            ...
            for item in data[items_name]:
                task = { 
                    ...
                    "topic": data['topic'],
                    "name": item[item_name],
                    "text_prompt": f"Расскажи про песню '{item[item_name]}' c альбома '{item[album_name]}' группы '{item[group_name]}', используй не более {TOPIC_WORD_LIMIT} слов, используй смайлики",
                    "image_prompt": f"Нарисуй рисунок, вдохновлённый песней '{item[item_name]}' c альбома '{item[album_name]}' группы '{item[group_name]}'",
